...
def _read_and_map_sbml_and_expression_data(model,genes_file,scaleName,scalingFactor,default_bound=1000000):
	...
	'''
	Smatrix, num_species, num_reactions, reaction_names, lbs, ubs, gene_associations = sbml_read.retrieve_information_from_SBML(model,default_bound)
	rxn_mean_expression, rxn_stdev_expression = _read_gene_expression_data(genes_file,gene_associations)
	...
	rxn_mean_expression = [(x / rxn_mean_expression[reaction_index_to_scale])*scalingFactor for x in rxn_mean_expression]
	rxn_stdev_expression = [(x / rxn_stdev_expression[reaction_index_to_scale]) for x in rxn_stdev_expression] #This does nothing!
...
def _read_gene_expression_data(genes_file, gene_associations):
	...
	rxn_mean_expression, rxn_stdev_expression = gene_reaction_mapper.map_genes_to_reactions(gene_associations, gene_to_expression, gene_to_expression_sd)
...
def _read_gene_relative_data(genes_file, gene_associations):
	...
	rxn_ratio, _ = gene_reaction_mapper.map_genes_to_reactions(gene_associations, gene_to_expression, gene_to_expression_sd)
