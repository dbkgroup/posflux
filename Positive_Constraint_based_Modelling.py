...
def _buildPositiveFBA(S,lbsIn,ubsIn,objective,default_bound=1000000):
	...
	thousands_reactions_by_reactions = default_bound * ones_reactions_by_reactions #note that this should change if limits change

	Pstack1 = hstack([S,-1*S,zeros_species_by_reactions,zeros_species_by_reactions,zeros_species_by_reactions])
	Pstack2 = hstack([negOnes_reactions_by_reactions,zeros_reactions_by_reactions,thousands_reactions_by_reactions,zeros_reactions_by_reactions,zeros_reactions_by_reactions])
	Pstack3 = hstack([zeros_reactions_by_reactions,negOnes_reactions_by_reactions,zeros_reactions_by_reactions,thousands_reactions_by_reactions,zeros_reactions_by_reactions])
	Pstack4 = hstack([zeros_reactions_by_reactions,zeros_reactions_by_reactions,ones_reactions_by_reactions,ones_reactions_by_reactions,zeros_reactions_by_reactions])
	Pstack5 = hstack([ones_reactions_by_reactions,ones_reactions_by_reactions,zeros_reactions_by_reactions,zeros_reactions_by_reactions,negOnes_reactions_by_reactions])
...
def _buildComparisonDaaave(S,lbs1In,ubs1In,lbs2In,ubs2In,target,default_bound=1000000):
	...
	#The easiest way to write this formulation is to concatenate S and -S to double the variables. The presolve deals with this very rapidly, so it's not worth the hassle of the extra code
	sDimensions = S.get_shape()
	num_species, num_reactions = sDimensions
	...
	Mstack1 = hstack([target_reactions_by_reactions,zeros_reactions_by_reactions,zeros_reactions_by_reactions,zeros_reactions_by_reactions,zeros_reactions_by_reactions,negOnes_reactions_by_reactions,negOnes_reactions_by_reactions,zeros_reactions_by_reactions])
	Mstack2 = hstack([negTarget_reactions_by_reactions,zeros_reactions_by_reactions,zeros_reactions_by_reactions,zeros_reactions_by_reactions,zeros_reactions_by_reactions,ones_reactions_by_reactions,zeros_reactions_by_reactions,negOnes_reactions_by_reactions])
	
	#Vertical stack
	MOMA_block = vstack([Mstack1,Mstack2])
	
	print '\tCombine blocks'
	zeros_upper_right = csr_matrix((num_species+(4*num_reactions),7*num_reactions))
	zeros_lower_left = csr_matrix((2*num_reactions,4*num_reactions))
	zeros_middle_left = csr_matrix((num_species+(4*num_reactions),5*num_reactions))
	zeros_middle_right = csr_matrix((num_species+(4*num_reactions),2*num_reactions))
	
	upper_stack = hstack([PosFBA_block1,zeros_upper_right])
	middle_stack = hstack([zeros_middle_left,PosFBA_block2,zeros_middle_right])
	...
	for row in range(len(target)):
		if math.isnan(target[row]):
			lower_stack.data[lower_stack.indptr[row]:lower_stack.indptr[row+1]] = float(0)								#Clear constraints on excesses
			lower_stack.data[lower_stack.indptr[row+num_reactions]:lower_stack.indptr[row+1+num_reactions]] = float(0)	#Clear constraints on deficits
			count += 1
	...
	equalities = stack1_eqs + stack2_eqs + stack3_eqs + stack4_eqs + stack5_eqs + stack6_eqs + stack7_eqs + stack8_eqs + stack9_eqs + stack10_eqs + stack11_eqs + stack12_eqs
	...
	RHS = stack1_RHS + stack2_RHS + stack3_RHS + stack4_RHS + stack5_RHS + stack6_RHS + stack7_RHS + stack8_RHS + stack9_RHS + stack10_RHS + stack11_RHS + stack12_RHS
...
def _buildSuperDaaave(S,lbsIn,ubsIn,target,stdevs,default_bound=1000000):
	...
	print '\tBuild MOMA block'
	Mstack1 = hstack([ones_reactions_by_reactions,negOnes_reactions_by_reactions,negOnes_reactions_by_reactions,zeros_reactions_by_reactions])
	Mstack2 = hstack([negOnes_reactions_by_reactions,ones_reactions_by_reactions,zeros_reactions_by_reactions,negOnes_reactions_by_reactions])
	MOMA_block = vstack([Mstack1,Mstack2])
	
	print '\tCombine blocks'
	zeros_upper_right = csr_matrix((num_species+(4*num_reactions),3*num_reactions))
	zeros_lower_left = csr_matrix((2*num_reactions,4*num_reactions))
	...
	stack7_eqs = ['<'] * num_reactions 
	equalities = stack1_eqs + stack2_eqs + stack3_eqs + stack4_eqs + stack5_eqs + stack6_eqs + stack7_eqs
	...
	stack7_RHS = [0.0] * num_reactions
	RHS = stack1_RHS + stack2_RHS + stack3_RHS + stack4_RHS + stack5_RHS + stack6_RHS + stack7_RHS
	...
	#Process target bounds to remove NaNs. Matrix block structure is maintained, meaning NaN variables are completely free and some constraints are inactive. These are left to the presolver to deal with.
	lTarget = []
	uTarget = []
	nanCount = 0
	for t in target:
	...
	for v in range(6*num_reactions, 7*num_reactions):
		if math.isnan(stdevs[fetch]) == False:
			std = stdevs[fetch]
			obj = 1 / (std + (minimum_stdev/2)) 	#This accounts for STDEV and zeros
			objective[v] = obj
