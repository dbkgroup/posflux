...
Import SBML file and return all lists and matrices (including S matrix)
required for constraint-based modelling
Import gene expression data from genes_file, a tab-delimited file of three
columns with headings:
gene	mean	std

Merge these such that a new lists are created of size num_reactions
(rxn_exp, rxn_exp_sds)

Returns:
Smatrix (sparse CSR), num_species (int), num_reactions (int),
reaction_names (list), lbs (list), ubs (list), rxn_mean_expression (list),
rxn_stdev_expression (list)
'''
import csv
import math
...
def read_and_map_sbml_and_expression_data(model, genes_file, scale_name,
                                          scale_factor, default_bound=10000):
    '''
    model = sbml file
    ...
    '''
    s_matrix, num_species, num_reactions, reaction_names, lbs, ubs, \
        gene_associations = sbml_read.retrieve_information_from_sbml(
            model, default_bound)
...
def _read_gene_expression_data(genes_file, gene_associations):
    ...
    with open(genes_file, 'rU') as csvfile:
        ...
        rxn_mean_expression, rxn_stdev_expression = \
            gene_reaction_mapper.map_genes_to_reactions(
